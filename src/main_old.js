...
function init() {
  ...
  Promise.all([
    ...
    for (let i = 0; i < markersCount; i++) {
      ...
      // // Create a dynamic line for the trail
      // const trailGeo = new THREE.BufferGeometry();
      // // Pre-allocate positions for the trail
      // const positions = new Float32Array(maxTrailLength * 3);
      // trailGeo.setAttribute('position', new THREE.BufferAttribute(positions, 3));
      // // Using a material with some transparency for a fading effect
...
function animate() {
  ...
  if (markerData.length > 0) {
    ...
    //   // Update the trail for this marker:
    //   // Shift existing trail positions to "age" them and append the new position
    //   const trail = trails[i];
    //   const posArray = trail.positions;
    //   const stride = 3; // x, y, z per point

    //   // Shift positions to make room for the new one
    //   for (let j = 0; j < (trail.count - 1) * stride; j++) {
    //     posArray[j] = posArray[j + stride];
    //   }
    //   // Append the current position at the end of the trail buffer
    //   const baseIndex = (trail.count - 1) * stride;
    //   posArray[baseIndex] = pos.x;
    //   posArray[baseIndex + 1] = pos.y;
    //   posArray[baseIndex + 2] = pos.z;

    //   // Increase the count until the trail is full
    //   if (trail.count < maxTrailLength) {
    //     trail.count++;
    //   }
    //   // Update the geometry so that the line shows only the valid part of the buffer
    //   trail.line.geometry.setDrawRange(0, trail.count);
    //   trail.line.geometry.attributes.position.needsUpdate = true;
